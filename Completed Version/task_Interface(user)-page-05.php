...
// var_dump("$username");
$sql = "SELECT * FROM `project` WHERE `proj_name` = '$proj_name' AND `username` = '$username'";
// var_dump($sql);
...
<body>
    ...
    <div class="w3-padding">
        ...
        <?php
    echo "
        <a href='/public_html/CreateTask.php?proj_name=" . $row['proj_name'] . "' class='w3-button w3-amber w3-ripple w3-round-xxlarge w3-right'>Create Task</a>
        <a href='/public_html/user_project_interface-page04.php' class='w3-button w3-amber w3-ripple w3-round-xxlarge w3-right'>Go Back</a>";
    ...
    <div class="w3-row-padding w3-border">
        <div class="w3-quarter list w3-padding">
            <div class="a.task" style="width=120%">
                ...
                if ($result = mysqli_query($conn, $sql1)) {
                    ...
                    if ($projectsCount > 0) {
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            echo "
                <div class='task-box'>
                    <a href='taskU.php?proj_name=$proj_name&task_num=$task_num' class='task'
                        <h4>" . ($row['task_name']) . "</h4>
                        <h4>" . ($row['date']) . "</h4>
                        <div>
                    ...
                    <select class='changeStatusU' onchange='location = this.value'>
                        <option class='no-display' selected='selected'>Status</option>
                        <option value='changeStatusU.php?proj_name=$proj_name&task_name=$task_num&status=1'>Backlogged</option>
                        <option value='changeStatusU.php?proj_name=$proj_name&task_name=$task_num&status=2'>Stagnent</option>
                        <option value='changeStatusU.php?proj_name=$proj_name&task_name=$task_num&status=3'>In-Progress</option>
                        <option value='changeStatusU.php?proj_name=$proj_name&task_name=$task_num&status=4'>Completed</option>
                    </select>
                </div>
                ";
                        }
